...
if has('gui_running') || has('termguicolors') || &t_Co == 88 || &t_Co == 256
  ...
  if &background ==# 'dark'
    ...
    let s:hue_7   = ['#af00af', '125'] " DeepPink4
    ...
    let s:pmenu        = ['#282c34', '16']
    let s:pmenu_sel    = ['#d19a66', '16']
    let s:cursorlinebg = s:syntax_cursor
    let s:normal       = ['#d7d7af', '185']
  else
  ...
  let s:syntax_fg = s:mono_3
  let s:syntax_fold_bg = s:mono_3

  " }}}

  " Vim editor color --------------------------------------------------------{{{
  call <sid>X('Normal',       s:normal,     s:syntax_bg,      '')
  ...
  call <sid>X('PMenuSel',     s:vertsplit,     s:pmenu_sel,         '')
  call <sid>X('PMenuSbar',    '',              s:mono_4,      '')
  call <sid>X('PMenuThumb',   '',              s:hue_2,         '')
  call <sid>X('Question',     s:hue_2,         '',               '')
  call <sid>X('Search',       s:syntax_bg,     s:hue_6_2,        '')
  call <sid>X('SpecialKey',   s:special_grey,  '',               'none')
  call <sid>X('Whitespace',   s:syntax_bg,  '',               'none')
  call <sid>X('StatusLine',   s:syntax_accent,     s:none,  'none')
  call <sid>X('StatusLineNC', s:mono_3,        '',               '')
  call <sid>X('TabLine',      s:none,          s:none,    'none')
  call <sid>X('TabLineFill',  s:mono_3,        s:none,    'none')
  ...
  call <sid>X('WarningMsg',   s:hue_5,         s:none,               '')
  call <sid>X('TooLong',      s:hue_5,         '',               '')
  call <sid>X('WildMenu',     s:syntax_fg,     s:mono_3,         '')
  call <sid>X('SignColumn',   s:none,          s:none,           '')
  ...
  call <sid>X('Comment',        s:hue_4,         '',          s:italic)
  call <sid>X('Constant',       s:hue_3,         '',          '')
  call <sid>X('String',         s:hue_2,         '',          '')
  call <sid>X('Character',      s:hue_1,         '',          '')
  ...
  call <sid>X('Statement',      s:hue_7,         '',          'none')
  call <sid>X('Conditional',    s:hue_8,         '',          '')
  call <sid>X('Repeat',         s:hue_6_2,       '',          '')
  call <sid>X('Label',          s:hue_5_2,       '',          '')
  ...
  call <sid>X('PreCondit',      s:hue_5_2,       '',          '')
  call <sid>X('Type',           s:hue_7,         '',          'none')
  call <sid>X('StorageClass',   s:hue_7,         '',          '')
  call <sid>X('Structure',      s:hue_8,         '',          '')
  ...
  " Diff highlighting -------------------------------------------------------{{{
  call <sid>X('DiffAdd',     s:hue_4, s:none, '')
  call <sid>X('DiffChange',  s:hue_6, s:none, '')
  call <sid>X('DiffDelete',  s:hue_5, s:none, '')
  call <sid>X('DiffText',    s:hue_2, s:none, '')
  call <sid>X('DiffAdded',   s:hue_4, s:none, '')
  call <sid>X('DiffFile',    s:hue_5, s:none, '')
  call <sid>X('DiffNewFile', s:hue_4, s:none, '')
  call <sid>X('DiffLine',    s:hue_2, s:none, '')
  call <sid>X('DiffRemoved', s:hue_5, s:none, '')
  " }}}
  ...
  " JSON highlighting -------------------------------------------------------{{{
  call <sid>X('jsonCommentError',         '',   '', ''        )
  call <sid>X('jsonKeyword',              '',   '', ''        )
  call <sid>X('jsonQuote',                '',   '', ''        )
  call <sid>X('jsonTrailingCommaError',   '',   '', 'reverse' )
  call <sid>X('jsonMissingCommaError',    '',   '', 'reverse' )
  call <sid>X('jsonNoQuotesError',        '',   '', 'reverse' )
  call <sid>X('jsonNumError',             '',   '', 'reverse' )
  call <sid>X('jsonString',               '',   '', ''        )
  call <sid>X('jsonBoolean',              '',   '', ''        )
  call <sid>X('jsonNumber',               '',   '', ''        )
  call <sid>X('jsonStringSQError',        '',   '', 'reverse' )
  call <sid>X('jsonSemicolonError',       '',   '', 'reverse' )
  " }}}
  ...
  call <sid>X('rustDeriveTrait',          s:hue_3,    '', '')
  call <sid>X('SpecialComment',           s:hue_1,    '', '')
  call <sid>X('rustCommentLine',          s:hue_4,    '', '')
  call <sid>X('rustCommentLineDoc',       s:hue_6,    '', '')
  call <sid>X('rustCommentLineDocError',  s:hue_6_2,    '', '')
  call <sid>X('rustCommentBlock',         s:hue_7,    '', '')
  call <sid>X('rustCommentBlockDoc',      s:hue_8,    '', '')
  call <sid>X('rustCommentBlockDocError', s:hue_4,    '', '')
  " }}}
